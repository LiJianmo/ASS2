...
public class TrainLine {
	...
	public TrainStation getNext(TrainStation station) {
		...
		for(int i=0;i<lineMap.length;i++) {
			if(lineMap[i].getName().contentEquals(station.getName())) flag=true;
		}
		if(!(flag)) throw new StationNotFoundException("NOT ON THIS LINE");
		if(goingRight) return station.getRight();
	...
	public TrainStation findStation(String name) {
		for(int i=0;i<lineMap.length;i++) {
			if(lineMap[i].getName().contentEquals(name)) return lineMap[i];
		}
		throw new StationNotFoundException("DON'T FIND!"); 
	}
	...
	public TrainStation[] getLineArray() {
		TrainStation[] LineArray=new TrainStation[lineMap.length];
		TrainStation Add_One=this.leftTerminus;
		for(int i=0;i<lineMap.length;i++) {
			LineArray[i]=Add_One;
			Add_One=Add_One.getRight();
		}
		return LineArray;
	}
	...
	private TrainStation[] shuffleArray(TrainStation[] array) {
		...
		for (int i = 0; i < array.length; i++) {
			...
			array[i] = temp;//swap the ele of array[i] and array[random]???
		}
		this.lineMap = array;
		return array;
	}
